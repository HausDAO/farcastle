...
The following tools provide automation around invite flows by making a bot your channel a moderator to check token balances before inviting members.

## Note on DAO Tokens

While the tools included here include criteria for invite automation, this guide outlines how to set them up using ERC-20 Moloch DAO tokens. This could also be done with an NFT or other token.

Moloch DAOs are a two token system and either can be used for gating a channel.

- **Voting Tokens** (shares) confer voting rights and a proportional ownership stake in the DAO's treasury.
  Tokens that
...
- Set-up [Cura Auto Invites](/scrolls/cura) to run in the background and send invites to potential members without them needing to find your channel first.
